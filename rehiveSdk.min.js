...
(function(){var baseAPI='https://rehive.com/api/3/',loginAPI='auth/login/',token='',header={header:'Content-Type: application/json'};function setToken(newToken){token=newToken;}
    function removeToken(){token="";}
    function postToRehive(url,data,cb){axios.post(baseAPI+url,data,header).then(function(response){cb(null,response);}).catch(function(error){cb(error.response,null);});}
    function login(identifier,company_id,password,cb){var data={identifier:identifier,company_id:company_id,password:password};postToRehive(loginAPI,data,cb);}
    var REHIVE={login:login,setToken:setToken};return window.REHIVE=REHIVE;})();function Login(identifier,company_id,password){REHIVE.login(identifier,company_id,password,function(err,res){if(err){console.log(err);return}
    if(res.status==200){REHIVE.setToken(res.data.data.token);window.alert('User logged in successfully');}})}